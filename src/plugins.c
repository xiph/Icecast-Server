...
{
#ifdef HAVE_ROARAUDIO
    ICECAST_LOG_DEBUG("Plugin Interface is being initialized");
    roarapi_lock();
    ...
    ICECAST_LOG_DEBUG("Plugin Interface is now initialized");
#endif
}
...
{
#ifdef HAVE_ROARAUDIO
    ICECAST_LOG_DEBUG("Plugin Interface is being shut down");
    roarapi_lock();
    ...
    sched = NULL;
    roarapi_unlock();
    ICECAST_LOG_DEBUG("Plugin Interface is now shut down");
#endif
}
...
#else
    ICECAST_LOG_ERROR("Can not load plugin \"%s\" as RoarAudio support is not compiled in.", plugin->name);
#endif
}
...
#else
    ICECAST_LOG_ERROR("Can not load CPI as RoarAudio support is not compiled in.");
#endif
}
