        ...
        ice_config_t *configuration)
{
    ...
    do {
        ...
        if (xmlStrcmp (node->name, XMLSTR("location")) == 0) {
            ...
            ICECAST_ICECAST_LOG_WARN("<source-password> defined outside <authentication>. This is deprecated.");
        ...
        ice_config_t *configuration)
{
    ...
    if (tmp) {
	    ...
	    ICECAST_ICECAST_LOG_WARN("Unknown mountpoint type: %s", tmp);
            config_clear_mount (mount);
            return;
	}
	xmlFree(tmp);
    ...
    {
    	ICECAST_ICECAST_LOG_WARN("Default mount %s has mount-name set. This is not supported. Behavior may not be consistent.", mount->mountname);
    }
    ...
    {
        ICECAST_ICECAST_LOG_WARN("Config for mount %s contains fallback options but no fallback mount.", mount->mountname);
    }
    ...
    do {
        ...
        if (xmlStrcmp (node->name, XMLSTR("source-password")) == 0) {
            if (xmlGetProp(node, XMLSTR("mount"))) {
                ICECAST_ICECAST_LOG_ERROR("Mount level source password defined within global <authentication> section.");
            }
            else {
        ...
        ice_config_t *configuration)
{
    ...
    if (configuration->num_yp_directories >= MAX_YP_DIRECTORIES) {
        ICECAST_ICECAST_LOG_ERROR("Maximum number of yp directories exceeded!");
        return;
    }
    do {
        if (node == NULL) break;
