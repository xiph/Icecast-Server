...
#ifdef _WIN32
#define source_run_script(x,y)  ICECAST_ICECAST_LOG_WARN("on [dis]connect scripts disabled");
#else
...
source_t *source_reserve (const char *mount)
{
    ...
    if(mount[0] != '/')
        ICECAST_ICECAST_LOG_WARN("Source at \"%s\" does not start with '/', clients will be "
...
void source_clear_source (source_t *source)
{
    ...
    ICECAST_ICECAST_LOG_DEBUG("clearing source \"%s\"", source->mount);
    ...
    if (source->dumpfile)
    {
        ICECAST_ICECAST_LOG_INFO("Closing dumpfile for %s", source->mount);
    ...
    if (c)
    {
        ...
        ICECAST_ICECAST_LOG_INFO("%d active listeners on %s released", c, source->mount);
    }
...
{
    ICECAST_ICECAST_LOG_DEBUG("freeing source \"%s\"", source->mount);
    avl_tree_wlock (global.source_tree);
...
void source_move_clients (source_t *source, source_t *dest)
{
    ...
    {
        ICECAST_ICECAST_LOG_WARN("src and dst are the same \"%s\", skipping", source->mount);
        return;
    }
    ...
    {
        ICECAST_ICECAST_LOG_WARN("destination mount %s not running, unable to move clients ", dest->mount);
    ...
    do
    {
        ...
        {
            ICECAST_ICECAST_LOG_INFO("source mount %s is not available", source->mount);
            break;
        }
        if (source->format && dest->format)
        {
            ...
            {
                ICECAST_ICECAST_LOG_WARN("stream %s and %s are of different types, ignored", source->mount, dest->mount);
                break;
            }
        }
        ...
        ICECAST_ICECAST_LOG_INFO("passing %lu listeners to \"%s\"", count, dest->mount);
...
static refbuf_t *get_next_buffer (source_t *source)
{
    ...
    while (global.running == ICECAST_RUNNING && source->running)
    {
        ...
        if (fds < 0)
        {
            ...
            {
                ICECAST_ICECAST_LOG_WARN("Error while waiting on socket, Disconnecting source");
                source->running = 0;
            }
            break;
        }
        if (fds == 0)
        {
            ...
            {
                ICECAST_ICECAST_LOG_DEBUG("last %ld, timeout %d, now %ld", (long)source->last_read,
                        source->timeout, (long)current);
                ICECAST_ICECAST_LOG_WARN("Disconnecting source due to socket timeout");
                source->running = 0;
            }
        ...
        {
            ICECAST_ICECAST_LOG_INFO("End of Stream %s", source->mount);
            source->running = 0;
            continue;
        }
        if (refbuf)
...
static void send_to_listener (source_t *source, client_t *client, int deletion_expected)
{
    ...
    while (1)
    {
        ...
        if (client->con->discon_time)
            ...
            {
                ICECAST_ICECAST_LOG_INFO("time limit reached for client #%lu", client->con->id);
                client->con->error = 1;
            }
    ...
    {
        ICECAST_ICECAST_LOG_INFO("Client %lu (%s) has fallen too far behind, removing",
...
static void source_init (source_t *source)
{
    ...
    if (source->dumpfilename != NULL)
    {
        ...
        {
            ICECAST_ICECAST_LOG_WARN("Cannot open dump file \"%s\" for appending: %s, disabling.",
    ...
    ICECAST_ICECAST_LOG_DEBUG("Source creation complete");
    ...
    while (global.running == ICECAST_RUNNING && source->running) {
        ...
        while (client_node) {
            ...
            if (client->con->error) {
                ...
                source->listeners--;
                ICECAST_ICECAST_LOG_DEBUG("Client removed");
                continue;
            }
            client_node = avl_get_next(client_node);
        }
        ...
        while (client_node) {
                    ...
                    source->listeners >= (unsigned long)source->max_listeners) 
            {
                ...
                ICECAST_ICECAST_LOG_INFO("Client deleted, exceeding maximum listeners for this "
            ...
            source->listeners++;
            ICECAST_ICECAST_LOG_DEBUG("Client added for mountpoint (%s)", source->mount);
        ...
        if (source->listeners != source->prev_listeners)
        {
            ...
            ICECAST_ICECAST_LOG_INFO("listener count on %s now %lu", source->mount, source->listeners);
        ...
        if (source->stream_data)
        {
            ...
            while (source->stream_data->_count == 1)
            {
                ...
                if (to_go->next == NULL || source->burst_point == to_go)
                {
                    ...
                    ICECAST_ICECAST_LOG_ERROR("queue state is unexpected");
                    source->running = 0;
                    break;
                }
...
static void source_shutdown (source_t *source)
{
    ...
    source->running = 0;
    ICECAST_ICECAST_LOG_INFO("Source from %s at \"%s\" exiting", source->con->ip, source->mount);
...
static void source_apply_mount (source_t *source, mount_proxy *mountinfo)
{
    ...
    ICECAST_ICECAST_LOG_DEBUG("Applying mount information for \"%s\"", source->mount);
    avl_tree_rlock (source->client_tree);
    stats_event_args (source->mount, "listener_peak", "%lu", source->peak_listeners);
    ...
    {
        ICECAST_ICECAST_LOG_DEBUG("YP changed to %d", val);
        if (val)
            yp_add (source->mount);
        else
            yp_remove (source->mount);
    ...
    if (mountinfo && mountinfo->intro_filename)
    {
        ...
        if (path)
        {
            ...
            if (f)
                ...
                ICECAST_ICECAST_LOG_WARN("Cannot open intro file \"%s\": %s", path, strerror(errno));
            free (path);
        }
    }
...
void source_update_settings (ice_config_t *config, source_t *source, mount_proxy *mountinfo)
{
    ...
    if (source->fallback_mount)
        ICECAST_ICECAST_LOG_DEBUG("fallback %s", source->fallback_mount);
    if (mountinfo && mountinfo->intro_filename)
        ICECAST_ICECAST_LOG_DEBUG("intro file is %s", mountinfo->intro_filename);
    if (source->dumpfilename)
        ICECAST_ICECAST_LOG_DEBUG("Dumping stream to %s", source->dumpfilename);
    if (mountinfo && mountinfo->on_connect)
        ICECAST_ICECAST_LOG_DEBUG("connect script \"%s\"", mountinfo->on_connect);
    if (mountinfo && mountinfo->on_disconnect)
        ICECAST_ICECAST_LOG_DEBUG("disconnect script \"%s\"", mountinfo->on_disconnect);
    if (source->on_demand)
    {
        ICECAST_ICECAST_LOG_DEBUG("on_demand set");
    ...
    {
        stats_event_hidden (source->mount, NULL, 1);
        ICECAST_ICECAST_LOG_DEBUG("hidden from public");
    }
    else
    ...
    ICECAST_ICECAST_LOG_DEBUG("public set to %d", source->yp_public);
    ICECAST_ICECAST_LOG_DEBUG("max listeners to %ld", source->max_listeners);
    ICECAST_ICECAST_LOG_DEBUG("queue size to %u", source->queue_size_limit);
    ICECAST_ICECAST_LOG_DEBUG("burst size to %u", source->burst_size);
    ICECAST_ICECAST_LOG_DEBUG("source timeout to %u", source->timeout);
    ICECAST_ICECAST_LOG_DEBUG("fallback_when_full to %u", source->fallback_when_full);
    thread_mutex_unlock(&source->lock);
}
...
static void source_run_script (char *command, char *mountpoint)
{
    ...
    switch (external_pid)
    {
        case 0:
            ...
            {
                case -1:
                    ICECAST_ICECAST_LOG_ERROR("Unable to fork %s (%s)", command, strerror (errno));
                    break;
                case 0:  /* child */
                    ICECAST_ICECAST_LOG_DEBUG("Starting command %s", command);
                    execl (command, command, mountpoint, (char *)NULL);
                    ICECAST_ICECAST_LOG_ERROR("Unable to run command %s (%s)", command, strerror (errno));
                    exit(0);
                default: /* parent */
                    break;
            }
            exit (0);
        case -1:
            ICECAST_ICECAST_LOG_ERROR("Unable to fork %s", strerror (errno));
            break;
        default: /* parent */
...
static void *source_fallback_file (void *arg)
{
    ...
    do
    {
        ...
        if (file == NULL)
        {
            ICECAST_ICECAST_LOG_WARN("unable to open file \"%s\"", path);
            free (path);
            break;
        }
        free (path);
        ...
        if (source == NULL)
        {
            ICECAST_ICECAST_LOG_WARN("mountpoint \"%s\" already reserved", mount);
            break;
        }
        ICECAST_ICECAST_LOG_INFO("mountpoint %s is reserved", mount);
