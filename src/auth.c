...
static auth_client *auth_client_setup (const char *mount, client_t *client)
{
    ...
    do
    {
        ...
        ICECAST_ICECAST_LOG_INFO("unhandled authorization header: %s", header);
...
static void queue_auth_client (auth_client *auth_user, mount_proxy *mountinfo)
{
    ...
    else
    {
        ...
        {
            ICECAST_ICECAST_LOG_WARN("internal state is incorrect for %p", auth_user->client);
            return;
        }
        auth = auth_user->client->auth;
        thread_mutex_lock (&auth->lock);
    }
    ICECAST_ICECAST_LOG_DEBUG("...refcount on auth_t %s is now %d", auth->mount, auth->refcount);
    *auth->tailp = auth_user;
    auth->tailp = &auth_user->next;
    auth->pending_count++;
    ICECAST_ICECAST_LOG_INFO("auth on %s has %d pending", auth->mount, auth->pending_count);
...
void auth_release (auth_t *authenticator)
{
    ...
    ICECAST_ICECAST_LOG_DEBUG("...refcount on auth_t %s is now %d", authenticator->mount, authenticator->refcount);
...
static void auth_new_listener (auth_t *auth, auth_client *auth_user)
{
    ...
    {
        ICECAST_ICECAST_LOG_DEBUG("listener is no longer connected");
        client->respcode = 400;
    ...
    if (auth_postprocess_listener (auth_user) < 0)
    {
        auth_release (client->auth);
        client->auth = NULL;
        ICECAST_ICECAST_LOG_INFO("client %lu failed", client->con->id);
    }
}
...
static void stream_auth_callback (auth_t *auth, auth_client *auth_user)
{
    ...
    if (client->authenticated)
        ...
        ICECAST_ICECAST_LOG_WARN("Failed auth for source \"%s\"", auth_user->mount);
}
...
static void *auth_run_thread (void *arg)
{
    ...
    ICECAST_ICECAST_LOG_INFO("Authentication thread started");
    while (auth->running)
    {
        ...
        if (auth->head)
        {
            ...
            ICECAST_ICECAST_LOG_DEBUG("%d client(s) pending on %s", auth->pending_count, auth->mount);
            ...
            if (auth_user->process)
                ...
                ICECAST_ICECAST_LOG_ERROR("client auth process not set");
    ...
    ICECAST_ICECAST_LOG_INFO("Authenication thread shutting down");
    return NULL;
}
...
static int add_listener_to_source (source_t *source, client_t *client)
{
    ...
    do
    {
        ICECAST_ICECAST_LOG_DEBUG("max on %s is %ld (cur %lu)", source->mount,
        ...
        if (loop && source->fallback_when_full && source->fallback_mount)
        {
            ...
            if (!next) {
                ICECAST_ICECAST_LOG_ERROR("Fallback '%s' for full source '%s' not found", 
                        source->mount, source->fallback_mount);
                return -1;
            }

            ICECAST_ICECAST_LOG_INFO("stream full trying %s", next->mount);
            source = next;
            loop--;
            continue;
        }
    ...
    if (source->running == 0 && source->on_demand)
    {
        ...
        ICECAST_ICECAST_LOG_DEBUG("kicking off on-demand relay");
        source->on_demand_req = 1;
    }
    ICECAST_ICECAST_LOG_DEBUG("Added client to %s", source->mount);
    return 0;
}
...
static int add_authenticated_listener (const char *mount, mount_proxy *mountinfo, client_t *client)
{
    ...
    if (util_check_valid_extension (mount) == XSLT_CONTENT)
    {
        ...
        ICECAST_ICECAST_LOG_DEBUG("Stats request, sending XSL transformed stats");
    ...
    if (source)
    {
        ...
        avl_tree_unlock (global.source_tree);
        if (ret == 0)
            ICECAST_ICECAST_LOG_DEBUG("client authenticated, passed to source");
    }
    else
    {
        avl_tree_unlock (global.source_tree);
...
void auth_postprocess_source (auth_client *auth_user)
{
    ...
    {
        ICECAST_ICECAST_LOG_DEBUG("metadata request (%s, %s)", req, mount);
        admin_handle_request (client, "/admin/metadata");
    }
    else
    {
        ICECAST_ICECAST_LOG_DEBUG("on mountpoint %s", mount);
...
void auth_add_listener (const char *mount, client_t *client)
{
    ...
    if (mountinfo && mountinfo->auth)
    {
        ...
        {
            config_release_config ();
            ICECAST_ICECAST_LOG_WARN("too many clients awaiting authentication");
            client_send_403 (client, "busy, please try again later");
            return;
        }
        auth_user = auth_client_setup (mount, client);
        auth_user->process = auth_new_listener;
        ICECAST_ICECAST_LOG_INFO("adding client for authentication");
...
static int get_authenticator (auth_t *auth, config_options_t *options)
{
    if (auth->type == NULL)
    {
        ICECAST_ICECAST_LOG_WARN("no authentication type defined");
        return -1;
    }
    do
    {
        ICECAST_ICECAST_LOG_DEBUG("type is %s", auth->type);
        ...
        if (strcmp (auth->type, "url") == 0)
        {
            ...
            break;
#else
            ICECAST_ICECAST_LOG_ERROR("Auth URL disabled");
            return -1;
#endif
        }
        ...
        ICECAST_ICECAST_LOG_ERROR("Unrecognised authenticator type: \"%s\"", auth->type);
        return -1;
    } while (0);
        ...
        if (xmlStrcmp (current->name, XMLSTR("option")) == 0)
        {
            ...
            *next_option = opt;
            ...
            if (xmlStrcmp (current->name, XMLSTR("text")) != 0)
                ICECAST_ICECAST_LOG_WARN("unknown auth setting (%s)", current->name);
    }
...
int auth_stream_authenticate (client_t *client, const char *mount, mount_proxy *mountinfo)
{
    if (mountinfo && mountinfo->auth && mountinfo->auth->stream_auth)
    {
        ...
        ICECAST_ICECAST_LOG_INFO("request source auth for \"%s\"", mount);
        queue_auth_client (auth_user, mountinfo);
        return 1;
    }
    return 0;
...
{
    ICECAST_ICECAST_LOG_INFO("Auth shutdown");
}
