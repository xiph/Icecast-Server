...
static void kate_codec_free (ogg_state_t *ogg_info, ogg_codec_t *codec)
{
    ...
    ICECAST_ICECAST_LOG_DEBUG("freeing kate codec");
...
static refbuf_t *process_kate_page (ogg_state_t *ogg_info, ogg_codec_t *codec, ogg_page *page)
{
    ...
    while (ogg_stream_packetout (&codec->os, &packet) > 0)
    {
        if (!kate->headers_done)
        {
            ...
            {
                ogg_info->error = 1;
                ICECAST_ICECAST_LOG_WARN("problem with kate header");
                return NULL;
            }
            header_page = 1;
...
ogg_codec_t *initial_kate_page (format_plugin_t *plugin, ogg_page *page)
{
    ...
    ICECAST_ICECAST_LOG_DEBUG("checking for kate codec");
#ifdef HAVE_KATE
    ...
    ICECAST_ICECAST_LOG_INFO("seen initial kate header");
