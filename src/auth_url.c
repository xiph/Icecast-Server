...
static void auth_url_clear(auth_t *self)
{
    ...
    ICECAST_ICECAST_LOG_INFO("Doing auth URL cleanup");
    url = self->state;
    self->state = NULL;
...
static auth_result url_remove_listener (auth_client *auth_user)
{
    ...
    if (curl_easy_perform (url->handle))
        ICECAST_ICECAST_LOG_WARN("auth to server %s failed with %s", url->removeurl, url->errormsg);
...
static auth_result url_add_listener (auth_client *auth_user)
{
    ...
    if (res)
    {
        ICECAST_ICECAST_LOG_WARN("auth to server %s failed with %s", url->addurl, url->errormsg);
        return AUTH_FAILED;
    }
    /* we received a response, lets see what it is */
    if (client->authenticated)
        return AUTH_OK;
    ICECAST_ICECAST_LOG_INFO("client auth (%s) failed with \"%s\"", url->addurl, url->errormsg);
    return AUTH_FAILED;
}
...
static void url_stream_start (auth_client *auth_user)
{
    ...
    if (curl_easy_perform (url->handle))
        ICECAST_ICECAST_LOG_WARN("auth to server %s failed with %s", stream_start_url, url->errormsg);
...
static void url_stream_end (auth_client *auth_user)
{
    ...
    if (curl_easy_perform (url->handle))
        ICECAST_ICECAST_LOG_WARN("auth to server %s failed with %s", stream_end_url, url->errormsg);
...
static void url_stream_auth (auth_client *auth_user)
{
    ...
    if (curl_easy_perform (url->handle))
        ICECAST_ICECAST_LOG_WARN("auth to server %s failed with %s", url->stream_auth, url->errormsg);
}
...
int auth_get_url_auth (auth_t *authenticator, config_options_t *options)
{
    ...
    ICECAST_ICECAST_LOG_INFO("URL based authentication setup");
    return 0;
}
